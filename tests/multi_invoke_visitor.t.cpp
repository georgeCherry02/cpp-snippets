...
SCENARIO("Basic usage of the visitor") {
  GIVEN("A dispatcher with 3 unique signatures") {
    ...
    WHEN("The dispatcher is called on three occasions with these different "
         "signatures") {
  ...
  GIVEN("A variant and a dispatcher to handle said variant") {
    ...
    auto dispatcher = make_dispatcher(
        [&int_called](int) { int_called = true; },
        [&string_called](std::string) { string_called = true; });
  ...
  GIVEN("A variant and a dispatcher with functions allowing implicit cast of "
        "variant members") {
    ...
    auto dispatcher =
        make_dispatcher([&int_called](int) { int_called = true; },
                        [&double_called](double) { double_called = true; });
    WHEN("The dispatcher is called with the variant") {
      dispatcher(test_int);
      THEN("The variant is appropriately unpacked, and the multiple flags are "
           "modified") {
        CHECK(int_called);
        CHECK(double_called);
      }
  ...
  GIVEN("A variant and a dispatcher functions accepting references") {
    ...
    bool int_called, double_called;
    auto dispatcher = make_dispatcher(
        [&int_called](std::string &call_flag, int) {
          int_called = true;
          call_flag = "int called";
        },
        [&double_called](std::string &call_flag, double) {
          double_called = true;
          call_flag = "double called";
        });
    WHEN("The dispatcher is called with the variant") {
      std::string call_flag{};
      dispatcher(call_flag, test_int);
      THEN("The variant is appropriately unpacked and both functions are "
           "called reliant on implicit cast") {
        CHECK(int_called);
        CHECK(double_called);
      }
      THEN("The string is properly taken by reference and the order of "
           "resolution is that in which handlers are passed to the "
           "dispatcher's constructor") {
